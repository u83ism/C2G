...
このプロジェクトには **Astro Docs MCPサーバー** (`astro-docs`) が設定されている。

- Astroの実装・調査を行う際は、必ずMCPの `search_astro_docs` ツールで最新ドキュメントを参照すること
- APIの使い方・型・設定オプションなど、記憶に頼らずMCPで確認する
- 特にバージョン間の変更点（v5→v6など）はMCPで最新情報を取得する
